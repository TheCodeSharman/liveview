...
var createDebugger = /*@__PURE__*/getDefaultExportFromCjs(srcExports);
...
const CLIENT_ENTRY = path.resolve(__dirname, '../client/client.js');
path.dirname(CLIENT_ENTRY);
path.resolve(__dirname, '../client/env.js');
...
let __vite__injectQuery = (url, queryToInject) => url;
...
const fetchRemote = (filename) => {
    ...
    if (request.status !== 200) {
        ...
        debug(`Failed to load "${source.cyan(url)}" with status code ${source.yellow(request.status)}`);
...
function patchRequire() {
    ...
    const exclude = (filename, request) => {
    ...
    Module.prototype.require = function liveViewRequire(request, context) {
        let filename;
        if (request.startsWith(NATIVE_MODULE_PREFIX)) {
            ...
            const loaded = originalRequire.call(this, `/hyperloop/${request.slice(HYPERLOOP_PREFIX.length)}`, context);
            if (loaded) {
                return loaded;
            }
        }
        ...
        if (filename && !exclude(filename, request)) {
            ...
            debug('[fallthrough] %s', source.cyan(request));
        }
 ...
 */
function requiresEsModuleInterop(request) {
    return request.includes('build/.vite') || request.includes('.json');
}
const esModuleInterop = (module) => {
    const getOwnPropNames = (target) => Object.getOwnPropertyNames(target);
    const namedExports = (target) => getOwnPropNames(target).filter((n) => n !== '__esModule' && n !== 'default');
...
//# sourceMappingURL=liveview.bootstrap.js.map
