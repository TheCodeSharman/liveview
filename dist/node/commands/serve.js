...
const config = (logger, config, cli) => {
    ...
    cli.on('cli:pre-validate', (obj, callback) => {
    ...
    buildCommand = require(path_1.default.join(cli.sdk.path, 'cli', 'commands', 'build.js'));
...
const run = async (logger, config, cli, finished) => {
    ...
    try {
        ...
        if (force) {
            ...
            const platformBuildDir = path_1.default.join(projectDir, 'build', legacyPlatformName, 'build');
            if (fs_extra_1.default.existsSync(platformBuildDir)) {
                logger.info(`${chalk_1.default.green('[LiveView]')} Cleaning stale build artifacts ...`);
                await fs_extra_1.default.remove(platformBuildDir);
            }
        ...
        logger.info(`${chalk_1.default.green('[LiveView]')} Server ready`);
    }
    catch (e) {
        console.error(e);
        return finished(e);
 ...
 * @param cli Titanium CLI instance
 */
async function resetCliHooks(cli, platform) {
    cli.hooks = {
        scannedPaths: {},
        pre: {},
        post: {},
    ...
    await cli.scanHooks(path_1.default.join(cli.sdk.path, platform, 'cli/hooks'));
}
